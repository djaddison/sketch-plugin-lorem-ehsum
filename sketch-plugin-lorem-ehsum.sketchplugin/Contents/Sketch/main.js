...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  var page = document.currentPage(); // Create Text Layer

  var layer = MSTextLayer.new();
  layer.textBehaviour = 1; // 0: auto, 1: fixed

  layer.frame().setWidth(400);
  var ending = Math.random() > 0.5 ? ' eh?' : '.';
  var randomCount = 20 + Math.floor(Math.random() * 20);
  var sentence = lodash_shuffle__WEBPACK_IMPORTED_MODULE_0___default()(wordList).slice(0, randomCount).join(' ');
  var capitalized = sentence[0].toUpperCase() + sentence.slice(1) + ending;
